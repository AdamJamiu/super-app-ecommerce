...
export const changePassword = async ({ email, oldPassword, newPassword }) => {
  const res = await dashboardClient.post(`Authentication/ChangePassword`, {
    email,
    oldPassword,
    newPassword,
  });
  ...
  return useQuery({
    ...
    queryFn: async () =>
      await dashboardClient
        .get("Dashboard/GetUserDashboardInfo")
        .then(function (res) {
...
export const addProductToCart = async ({
  ...
  const res = await httpClient.post(`Cart/AddProductsToCart/${userId}`, [
    { productId, quantity },
  ]);
  return res.data;
};
...
export const removeProductFromCart = async ({ productId }) => {
  ...
  const res = await httpClient.post("Cart/RemoveProduct", {
    productId,
    userId,
  });
  return res.data;
...
export const updateCart = async ({ productId, quantity }) => {
  ...
  const response = await httpClient.post(
    `Cart/ChangeProductQuantity/${userId}`,
    { productId, quantity }
  );
  return response.data;
};
...
export const getDeliveryMethods = () => {
  return useQuery({
    ...
    queryFn: async () =>
      await dashboardClient
        .get(`Dashboard/GetDeliveryMethods`)
        .then((res) => res?.data?.data),
    retry: 4,
    refetchOnMount: false,
    refetchOnWindowFocus: false,
...
export const getPaymentMethods = () => {
  return useQuery({
    ...
    queryFn: async () =>
      await dashboardClient
        .get(`Dashboard/GetPaymentMethods`)
        .then((res) => res?.data?.data),
    retry: 4,
    refetchOnMount: false,
    refetchOnWindowFocus: false,
...
export const getAllOrdersByUserId = ({ userId, Classification }) => {
  return useQuery({
    ...
    queryFn: async () =>
      await dashboardClient
        .get(`Order/GetAllUserOrders/${userId}`, {
          params: {
            PageSize: 100,
            Classification,
          },
...
export const editUser = async ({
  ...
  const res = await httpClient.put(`Authentication/EditUser/${userId}`, {
    firstName,
    lastName,
    phoneNumber,
    userName,
...
export const reInitializePayment = async ({ setLoading, orderId }) => {
  ...
  await dashboardClient
    .post("Order/InitializePayment", { orderId, medium: "WEB", callback_url })
    .then((res) => {
      window.location.href = res?.data?.data;
    })
    .catch((err) => {
